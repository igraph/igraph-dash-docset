...
def download_release():
    ...
    with urllib.request.urlopen("https://api.github.com/repos/igraph/igraph/releases") as url:
...
def create_docset(docdir, docset_name='igraph'):
    ...
    for file in glob(os.path.join(htmldir, 'igraph-*.html')):
        ...
        for a in anchors:
            ...
            if name in docsyms:
                ...
                docsyms[name] = (name, kind, link)
                p = a.getparent()
                p.insert(p.index(a) + 1, fromstring("<a name='//apple_ref/cpp/%s/%s' class='dashAnchor' />" % (kind, name)))
    ...
    for triplet in docsyms.values():
        cur.execute('INSERT OR IGNORE INTO searchIndex(name, type, path) VALUES (?,?,?)', triplet)
...
def create_dash_submission(version, revision=0):
    ...
    open(os.path.join(subdir, 'docset.json'), 'w').write( tem.substitute(version=version, revision=revision) )
